...
class PedidoControllerTest extends PostgresTestContainer {
    ...
    @Test
    void postPedidoComClienteInexistenteDeveRetornarErro() throws Exception {
        // Cliente inexistente lança RuntimeException, traduzida para 404 pelo handler global.
        mockMvc.perform(post("/api/pedido")
                        .header("Authorization", "Bearer " + tokenVendedor)
                        .contentType(MediaType.APPLICATION_JSON)
                        .content("""
    ...
    void getMeusPedidosClienteRetornaApenasOsProprios() throws Exception {
        ...
        // O PedidoResponse expõe o cliente no campo "cliente" (com id/nome/email).
        for (JsonNode pedido : pedidos) {
            org.assertj.core.api.Assertions.assertThat(
                    pedido.get("cliente").get("id").asLong()
            ).isEqualTo(idCliente);
        }
    }
    ...
    private Long buscarIdPorEmail(String email) throws Exception {
        MvcResult result = mockMvc.perform(get("/api/usuario")
                        .header("Authorization", "Bearer " + tokenAdmin))
                .andReturn();
        // A listagem de usuários é paginada: os itens ficam em "content".
        JsonNode usuarios = mapper.readTree(result.getResponse().getContentAsString()).get("content");
