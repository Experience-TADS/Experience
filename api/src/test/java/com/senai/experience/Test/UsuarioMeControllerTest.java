...
class UsuarioMeControllerTest extends PostgresTestContainer {
    ...
    @Test
    void getMeSemTokenDeveRetornar403() throws Exception {
        // /api/usuario/me exige autenticação; sem token o Spring Security bloqueia com 403.
        mockMvc.perform(get("/api/usuario/me"))
                .andExpect(status().isForbidden());
    }

    @Test
    void getMeComTokenInvalidoDeveRetornar403() throws Exception {
        // Token malformado não autentica; o endpoint protegido responde 403.
        mockMvc.perform(get("/api/usuario/me")
    ...
    @Test
    void postUsuarioComEmailDuplicadoEhPermitido() throws Exception {
        // A entidade Usuario não define o email como único, então a API permite
        // cadastrar mais de um usuário com o mesmo email (ambos retornam 201).
    ...
    private Long buscarIdPorEmail(String email) throws Exception {
        ...
        // A listagem é paginada: os usuários ficam em "content".
        var usuarios = root.get("content");
        for (var u : usuarios) {
