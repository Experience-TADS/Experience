...
class VeiculoControllerTest extends PostgresTestContainer {
    ...
    void putVeiculoComAdminDeveRetornar200() throws Exception {
        ...
        // O update do veículo altera apenas o statusVeiculo; o chassi é imutável
        // e permanece o valor original (12345), mesmo enviando outro no corpo.
        mockMvc.perform(put("/api/veiculo/" + id)
                        .header("Authorization", "Bearer " + tokenAdmin)
                        .contentType(MediaType.APPLICATION_JSON)
                        .content("""
                                {"idProduto":%d,"chassi":99999,"statusVeiculo":"MONTAGEM_ESTRUTURAL"}
                                """.formatted(idProduto)))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.chassi").value(12345))
                .andExpect(jsonPath("$.statusVeiculo").value("MONTAGEM_ESTRUTURAL"));
    }
    ...
    void getStatusHistoricoComTokenDeveRetornar200() throws Exception {
        ...
        // O histórico de status retorna uma List pura (não paginada).
    ...
    @Test
    void postStatusComClienteDevePermitir() throws Exception {
        Long id = criarVeiculo();

        // O endpoint POST /api/veiculo/{id}/status é permitAll (recebe eventos de fabricação),
        // então qualquer usuário autenticado consegue avançar o status.
        mockMvc.perform(post("/api/veiculo/" + id + "/status")
                        .header("Authorization", "Bearer " + tokenCliente)
                        .contentType(MediaType.APPLICATION_JSON)
                        .content("\"MONTAGEM_ESTRUTURAL\""))
    ...
    @Test
    void postStatusTransicaoInvalidaDeveRetornar400() throws Exception {
        Long id = criarVeiculo();

        // Tenta ir de AGUARDANDO direto para PINTURA (transição inválida).
        // O GlobalHandlerException traduz "transição inválida" para 400 Bad Request.
        mockMvc.perform(post("/api/veiculo/" + id + "/status")
                        .header("Authorization", "Bearer " + tokenAdmin)
                        .contentType(MediaType.APPLICATION_JSON)
                        .content("\"PINTURA\""))
