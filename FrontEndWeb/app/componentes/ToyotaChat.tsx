...
const API = process.env.NEXT_PUBLIC_CHATBOT_URL || "http://localhost:5000/api";
  ...
  useEffect(() => {
    ...
    fetch(`${API}/start`)
      ...
      .catch(() => {
        setMessages([{
          from: "bot",
          text: "⚠️ Não foi possível conectar ao servidor do assistente.\nCertifique-se de que o servidor Python está rodando.",
        }]);
      });
  }, []);
  ...
  return (
    <div className="flex h-screen bg-gray-100 dark:bg-gray-950 overflow-hidden">
      ...
      <div className="flex-1 flex flex-col md:ml-20 overflow-hidden">
        ...
        <div className="flex-1 flex flex-col mx-5 md:mx-10 mb-6 bg-white dark:bg-gray-900 rounded-3xl shadow-md border border-gray-100 dark:border-gray-800 overflow-hidden">
          ...
          <div className="flex items-center gap-3 px-5 py-4 border-b border-gray-100 dark:border-gray-800 shrink-0">
            ...
            <div>
              <p className="text-sm font-semibold text-gray-900 dark:text-white leading-tight">Totoya</p>
              <div className="flex items-center gap-1.5 mt-0.5">
                <span className="w-1.5 h-1.5 rounded-full bg-green-400" />
                <p className="text-xs text-gray-400">Assistente de Segurança · Online</p>
              </div>
            </div>

            {userName && (
              <div className="ml-auto flex items-center gap-2">
                <p className="text-xs text-gray-400 hidden sm:block">{firstName}</p>
          ...
          <div className="px-5 md:px-8 py-4 border-t border-gray-100 dark:border-gray-800 bg-white dark:bg-gray-900 flex gap-3 items-center shrink-0">
            <input
              ref={inputRef}
              type="text"
              value={input}
              ...
              className="flex-1 bg-gray-100 dark:bg-gray-800 text-gray-900 dark:text-gray-100 placeholder-gray-400 dark:placeholder-gray-500 rounded-xl px-4 py-3 text-sm outline-none focus:ring-2 focus:ring-red-500 focus:bg-white dark:focus:bg-gray-700 border border-transparent focus:border-red-200 dark:focus:border-red-800 transition-all duration-200"
            />
            <button
              onClick={sendMessage}
              disabled={!input.trim()}
              className="bg-red-600 hover:bg-red-700 disabled:bg-gray-200 dark:disabled:bg-gray-700 disabled:text-gray-400 text-white p-3 rounded-xl transition-all duration-200 active:scale-95 shadow-sm hover:shadow-md disabled:shadow-none disabled:cursor-not-allowed"
              aria-label="Enviar"
            >
              <Send size={18} />
            </button>
